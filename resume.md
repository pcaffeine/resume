...
制御アルゴリズムは考えれるうち、もっとも単純な形式を用いた。その理由は本論文の目的がdisturbance observerを用いたsystemのロバスト性を示す第一歩にあり、transient responseの収束時間などを考慮していない。したがって、制御アルゴリズムは考えられるうち、最も単純な形式を用いた。ただし、そこを改善するために、いくつかの補題を追加すれば、ある種のnon-linear gain shceduling controllerのロバスト性を証明できる可能性があると予想する。
